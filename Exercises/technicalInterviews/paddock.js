...
const paddockTotalHectarea = () => {
	const hectareaOfPaddocks = {};

	paddocks.forEach(({ paddockTypeId, area }) => {
		hectareaOfPaddocks[paddockTypeId] ??= 0;
		hectareaOfPaddocks[paddockTypeId] += area;
	});
	return paddockType
		.map((paddockType) => ({
			...paddockType,
			area: hectareaOfPaddocks[paddockType.id],
		}))
