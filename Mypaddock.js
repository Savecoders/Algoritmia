...
const paddockTotalHectarea = () => {
	const objPaddockHectarea = {};
	//Transformor el arreglo a Objecto y obtengo el valor de las hectareas.
	paddocks.forEach(
		({ paddockTypeId, area }) =>
			(objPaddockHectarea[paddockTypeId] = !objPaddockHectarea[paddockTypeId]
				? area
				: (objPaddockHectarea[paddockTypeId] += area))
	);
	return paddockType
		.map((paddockOfType) => ({
			...paddockOfType,
			hectareas: objPaddockHectarea[paddockOfType.id],
		}))
		.sort((a, b) => b.hectareas - a.hectareas)
		.map(({ name }) => name);
};

const paddockListHectarea = paddockTotalHectarea();

console.log(paddockListHectarea);
